package com.example.invoiceapp.utils;

public class Constants {

	public static final int BREAD_LIST=100;
	public static final int PREVIEW_BREAD_LIST=101;
	public static final int SUCCESS=1000;
	public static final int FAILURE=1001;
	public static final int REQ_FETCH_DRIVERS=2000;
	public static final int REQ_FETCH_PRODUCTS=2001;
	public static final int REQ_FETCH_ROUTES=2002;
	public static final int REQ_FETCH_WAYPOINTS=2003;
	public static final int REQ_FETCH_REMINDERS=2004;
	
	public static final int DB_REQ_FETCH_CUSTOMERS=500;
	public static final int DB_REQ_CHECK_TABLE_NULL_NOT=501;
	public static final int DB_REQ_FETCH_DRIVERS=502;
	public static final int DB_REQ_FETCH_PRODUCTS=503;
	public static final int DB_REQ_FETCH_ORDERED_PRODUCTS=504;
	public static final int DB_REQ_FETCH_INVOICES=505;
	public static final int DB_REQ_FETCH_INVOICES_DETAILS=506;
	public static final int DB_REQ_FETCH_PURCHASED_DETAILS=507;
	public static final int DB_REQ_FETCH_PENDING_INVOICE_DETAILS=508;
	public static final int DB_REQ_FETCH_REMINDERS=509;
	
	public static final String CUSTOM_ACTION_INTENT="com.example.invoiceapp.FINISH_ACTIVITY_SERVICE";
	public static final String CUSTOM_HOME_ACTION_INTENT="com.example.invoiceapp.FINISH_ACTIVITY";
	
}
